                    ...
                    <div class="col-md-4">
                        ...
                        <form action="{{ route('orders.update', $order->id) }}" method="POST">
                            ...
                            <div class="input-group input-group-sm shadow-sm">
                                <span
                                    class="input-group-text border-0 @if($order->status == 'przyjęte') bg-warning-subtle text-warning @elseif($order->status == 'w przygotowaniu') bg-primary-subtle text-primary-emphasis @elseif($order->status == 'wysłano') bg-success-subtle text-success @elseif($order->status == 'dostarczono') bg-secondary-subtle text-secondary @else bg-warning-subtle text-warning @endif">
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor"
                                        ...
                                        <path
                                            d="M15.964.686a.5.5 0 0 0-.65-.65L.767 5.855H.766l-.452.18a.5.5 0 0 0-.082.887l.41.26.001.002 4.995 3.178 3.178 4.995.002.001.26.41a.5.5 0 0 0 .886-.083zm-1.833 1.89L6.637 10.07l-.215-.338a.5.5 0 0 0-.154-.154l-.338-.215 7.494-7.494 1.178-.471z" />
                                    </svg>
                                    @else
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor"
                                ...
                                <select name="status"
                                    class="fw-bold form-select bg-light border-0 @if($order->status == 'przyjęte') text-warning @elseif($order->status == 'w przygotowaniu') text-primary @elseif($order->status == 'wysłano') text-success @elseif($order->status == 'dostarczono') text-secondary @else text-warning @endif"
                                    onchange="this.form.submit()">

                                    <option value="przyjęte" {{ $order->status == 'przyjęte' ? 'selected' : '' }}>
                                        Przyjęte</option>
                                    <option value="w przygotowaniu"
                                        {{ $order->status == 'w przygotowaniu' ? 'selected' : '' }}>W przygotowaniu
                                    </option>
                                    <option value="wysłano" {{ $order->status == 'wysłano' ? 'selected' : '' }}>Wysłano
                                    </option>
                                    <option value="dostarczono" {{ $order->status == 'dostarczono' ? 'selected' : '' }}
                                        disabled>Dostarczono</option>
                                </select>
                            </div>
                        </form>
                    </div>
